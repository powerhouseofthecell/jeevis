...
function genGraph() {
    ...
    // Add a Hover-over Title to Nodes - this will always be the node's id
    node.append("title")
        .text(function (d) { return d.id; });

    // Node Text Label - this should allow for distance measurements
    node.append("text")
        .attr("dy", -8)
        .attr("dx", -6)
        .text(nodeLabelTextFunction);
...
function nodeLabelTextFunction (d) {
    if (calcDistance) {
        return dist[d.id];
    } else {
        return d.id;
